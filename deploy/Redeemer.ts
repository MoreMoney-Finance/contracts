...
const deploy: DeployFunction = async function ({
  ...
  const { deployer, curveZap } = await getNamedAccounts();
  ...
  const Redeemer = await deploy('Redeemer', {
    from: deployer,
    args: [curveZap, [dai, usdc, usdt], [1, 2, 3], roles.address],
    log: true,
    skipIfAlreadyDeployed: true
  });

  await manage(deployments, Redeemer.address, 'Redeemer');
};
deploy.tags = ['Redeemer', 'base'];
