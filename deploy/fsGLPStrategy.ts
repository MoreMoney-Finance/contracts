...
const deploy: DeployFunction = async function ({
  ...
  const fsGLPStrategy = await deploy('fsGLPStrategy', {
    from: deployer,
    args: [roles.address],
    log: true,
  ...
  await manage(deployments, fsGLPStrategy.address, 'fsGLPStrategy');
  await registerStrategy(deployments, fsGLPStrategy.address);
};
deploy.tags = ['fsGLPStrategy', 'avalanche'];
