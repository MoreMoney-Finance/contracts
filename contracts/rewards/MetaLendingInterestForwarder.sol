...
contract MetaLendingInterestForwarder is
    RoleAware,
    ...
    ReentrancyGuard
{
    ...
    constructor(address _iMoney, address _roles) RoleAware(_roles) {
        interestLastForwarded = MetaLending(Roles(_roles).mainCharacters(META_LENDING)).totalEarnedInterest();
    ...
    function forwardInterest() public nonReentrant {
        uint256 newInterest = metaLending().totalEarnedInterest();
    ...
    function claimableInterest() public view returns (uint256) {
        return metaLending().totalEarnedInterest() - interestLastForwarded;
    }
