    ...
    IMasterMore
{
    ...
    struct PoolInfo {
        ...
        uint256 adjustedAllocPoint; // Adjusted allocation points for this pool. MOREs to distribute per second.
    }
    ...
    function _updatePool(uint256 _pid) private {
        ...
        if (block.timestamp > pool.lastRewardTimestamp) {
            ...
            uint256 moreReward = (secondsElapsed *
                morePerSec *
                pool.adjustedAllocPoint) / totalAdjustedAllocPoint;
            // update accMorePerShare to reflect dialuting rewards
            pool.accMorePerShare +=
                (moreReward * 1e12 * dialutingRepartition) /
                (lpSupply * 1000);

            // update accMorePerFactorShare to reflect non-dialuting rewards
            if (pool.sumOfFactors == 0) {
