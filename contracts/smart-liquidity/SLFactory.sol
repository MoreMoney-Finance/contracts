...
contract SLFactory is RoleAware {
    ...
    function initPool(string calldata poolName, string calldata poolSymbol)
        external
        returns (address, address)
    {
        ...
        return (
            address(
                new AuxLPT(
                    SMART_LIQUIDITY,
                    string(abi.encodePacked("MONEY - ", poolName)),
                    string(abi.encodePacked("MLPT-", poolSymbol)),
                    address(roles)
                )
            ),
            address(
                new AuxLPT(
                    SMART_LIQUIDITY,
                    string(abi.encodePacked("COUNTER - ", poolName)),
                    string(abi.encodePacked("CLPT-", poolSymbol)),
                    address(roles)
                )
            )
        );
