...
contract AaveV3StrategyV1 is YakStrategyV2 {
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _rewardSwapPairs.length; i++) {
            _addReward(
                _rewardSwapPairs[i].reward,
                _rewardSwapPairs[i].swapPair
            );
        }
    ...
    function removeReward(address _rewardToken) public onlyDev {
        ...
        for (uint256 i = 0; i < supportedRewards.length; i++) {
            if (_rewardToken == supportedRewards[i]) {
                found = true;
                supportedRewards[i] = supportedRewards[
                    supportedRewards.length - 1
                ];
            }
        }
    ...
    function isPairEquals(
        ...
        return
            pair.token0() == address(left) && pair.token1() == address(right);
    }
    ...
    function withdraw(uint256 amount) external override {
        ...
        if (depositTokenAmount > 0) {
            ...
            uint256 withdrawnAmount = _withdrawDepositTokens(
                depositTokenAmount
            );
    ...
    function _getRedeemable(
        ...
        return
            balance
                ...
                .mul(1e18)
                .sub(
                    borrowed.mul(13).div(10).mul(1e18).div(threshold).div(
                        100000
                    )
                )
                .div(1e18);
